...
const InsertionSort = (function () {
     ...
     * @param  {function} compareFunction Specifies a function that defines the sort order
     * @yield {Array} the partially sorted array.
     * @return {Array} the sorted array.
     */
    const insertionSortGenerator = function* (compareFunction) {
        let result = this.slice(0); // Copy the array
        let valueToInsert;
    ...
    return {
        sort(compareFunction, useGenerator = false) {
            if (useGenerator) {
                return insertionSortGenerator.call(this, compareFunction);
            }
            return insertionSort.call(this, compareFunction);
        },
    };
}());
